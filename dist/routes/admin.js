export function registerAdminRoutes(app, prisma, logger) {
    // DEV ONLY: Direct user wipe without auth (for development reset)
    app.post('/dev/wipe-users', async (req, res) => {
    ...
    app.get('/api/admin/users/:userId/predictions', async (req, res) => {
        const user = req.authUser;
        if (!user || user.role !== 'ADMIN')
            return res.status(403).json({ error: 'FORBIDDEN' });
    ...
    app.post('/api/admin/users/:userId/predictions', async (req, res) => {
        const user = req.authUser;
        if (!user || user.role !== 'ADMIN')
            return res.status(403).json({ error: 'FORBIDDEN' });
        const { userId } = req.params;
    ...
    app.delete('/api/admin/users/:userId/predictions/:matchId', async (req, res) => {
        const user = req.authUser;
        if (!user || user.role !== 'ADMIN')
            return res.status(403).json({ error: 'FORBIDDEN' });
