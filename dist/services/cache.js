...
class DataCache {
     ...
     */
    async get(key, fetcher, ttlMs = 60000) {
     ...
     */
    setupAutoRefresh(key, fetcher, ttlMs) {
...
export function initializeCache(prisma, logger) {
    if (!cacheInstance) {
...
export class CachedDataService {
    ...
    prisma;
    logger;
    constructor(cache, prisma, logger) {
        this.cache = cache;
        this.prisma = prisma;
        this.logger = logger;
    }
     ...
     */
    async getMatches() {
        return this.cache.get('matches', async () => {
    ...
    async getLeaderboard() {
        return this.cache.get('leaderboard', async () => {
     ...
     */
    async syncMatchesFromAPI(season) {
