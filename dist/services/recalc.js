...
export async function recalcForMatch(prisma, matchId) {
    const match = await prisma.match.findUnique({ where: { id: matchId } });
    if (!match || match.scoreHome == null || match.scoreAway == null)
        return { updated: 0 };
    ...
    for (const p of preds) {
        ...
        if (!aggregates[p.userId])
            aggregates[p.userId] = { total: 0, exact: 0, diff: 0, outcome: 0 };
        aggregates[p.userId].total += pts;
        if (pts === 5)
            aggregates[p.userId].exact += 1;
        else if (pts === 3)
            aggregates[p.userId].diff += 1;
        else if (pts === 2)
            aggregates[p.userId].outcome += 1;
    }
    const ops = Object.entries(aggregates).map(([userId, a]) => prisma.score.upsert({
        where: { userId },
        create: {
            userId,
            pointsTotal: a.total,
...
export async function recalcAll(prisma) {
